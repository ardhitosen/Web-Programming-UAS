    ...
    <body>
        ...
        <div class="container mx-auto my-8">
            <div class="mb-8 bg-green-100 p-6 rounded text-green-600">
                {{ session('success') }}
            </div>
        </div>
    @endif
    ...
    @if (session('recovery_code'))
        <div class="container mx-auto my-8">
            <div class="mb-8 bg-yellow-100 p-6 rounded text-yellow-600">
                Recovery Code: {{ session('recovery_code') }}
            </div>
        </div>
    @endif

        <form action="/patients/loginProcess" method="post" class="max-w-md mx-auto my-8 p-6 bg-white rounded shadow form-container">
            @if ($errors->any())
            ...
            <a href="/patients/forgot" class="text-blue-500">Forgot Password?</a>

            <button type="submit"
                class="w-full bg-sky-500 text-white py-2 px-4 rounded hover:bg-blue-400 transition-colors">Login</button>
        </form>
        <div class="text-center mt-4">
            <p class="text-gray-700">Don't have an account? <a href="/patients/create" class="text-blue-500">Register here</a></p>
            <p class="text-gray-700">Login as admin instead? <a href="/admins/login" class="text-blue-500">Login here</a></p>          
        </div>
    </body>
    </html>
